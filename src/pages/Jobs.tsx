            ...
            <Card key={job.id} className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate(`/job/${job.id}`)}>
              <CardHeader>
                ...
                <CardDescription className="flex items-center gap-2">
                  ...
                  {job.department || job.company_name || "Growth Accelerator"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <MapPin className="h-4 w-4" />
                  {job.city && job.state ? `${job.city}, ${job.state}` : job.location_name || "Remote"}
                </div>
      ...
      {jobs.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Try adjusting your search terms or check back later for new job postings.
          </p>
        </div>
      )}
